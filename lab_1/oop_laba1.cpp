...
int main() {
    int number;  // объявляем переменную name
    std::cout << "What is your number\n";
    std::cin >> number;  // считываем её значение с клавиатуры
    std::cout << "(" << closestPairToNum(number).first << ", " << closestPairToNum(number).second << ")" << std::endl;    
    return 0;
}
